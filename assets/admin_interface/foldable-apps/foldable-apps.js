(function () {
  window.onload = function () {
    for (let moduleEl of document.querySelectorAll(
      ...
      if (moduleAppClass) {
        let moduleAppKey =
          "admin-interface.foldable-apps_" + moduleAppClass + "_collapsed";
        let moduleCollapsed = Boolean(
          parseInt(localStorage.getItem(moduleAppKey) || 0) || 0,
        );
        ...
        for (let captionEl of moduleEl.querySelectorAll("caption")) {
          captionEl.onclick = function (event) {
            ...
            if (event.target.tagName.toLowerCase() === "caption") {
              ...
              moduleCollapsed =
                moduleEl.classList.contains(moduleCollapsedClass);
