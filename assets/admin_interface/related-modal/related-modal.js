  ...
  (function ($) {
    $(document).ready(function () {
      ...
      function dismissRelatedLookupModal(win, chosenId) {
        ...
        const widgetName = windowName.replace(
          /^(change|add|delete|lookup)_/,
          "",
        );
        const widgetEl = $("#" + widgetName);
        const widgetVal = widgetEl.val();
        if (
          widgetEl.hasClass("vManyToManyRawIdAdminField") &&
          Boolean(widgetVal)
        ) {
      ...
      function presentRelatedObjectModal(e) {
        ...
        const iframeHTML =
          ...
          '"></iframe>';
        const modalHTML =
          '<div class="related-modal-iframe-container">' +
          iframeHTML +
          "</div>";
      ...
      window.presentRelatedObjectModalOnClickOn =
      ...
      presentRelatedObjectModalOnClickOn(
        "a.dynamic_raw_id-related-lookup",
        true,
      );
      ...
      presentRelatedObjectModalOnClickOn(
        '.streamfield_app a.stream-btn[href*="_popup=1"]',
      );
